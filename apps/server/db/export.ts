// Environment variables provided via cross-env
import {
  db,
  users,
  courses,
...
async function exportDatabase() {
  ...
  try {
    ...
    fs.writeFileSync(
      path.join(exportDir, "lessons.json"),
    ...
    fs.writeFileSync(
      path.join(exportDir, "enrollments.json"),
    ...
    fs.writeFileSync(
      path.join(exportDir, "lesson_progress.json"),
      JSON.stringify(lessonProgressData, null, 2)
    );
    ...
    fs.writeFileSync(
      path.join(exportDir, "certificates.json"),
